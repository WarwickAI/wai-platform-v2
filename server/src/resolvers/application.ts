...
} from "../utils/RoleApplicationInput";
...
export class ApplicationResolver {
  ...
  async roleApplications(): Promise<RoleApplication[]> {
    return await RoleApplication.find({
      where: { display: true },
      relations: ["role"],
    });
  }
  ...
  async roleApply(
    ...
    if (!user) {
      return {
        errors: [{ field: "User ID", message: "No user found with ID" }],
      };
    }
    ...
    if (
      roles.findIndex((role) =>
        roleId ? role.id === roleId : role.shortName === roleShortName
      ) !== -1
    ) {
      return {
        errors: [
    ...
    if (!role.canApply) {
      return {
        errors: [
          {
            field: "Role ID/Short Name",
            message: "Role not accepting applications",
          },
        ],
      };
    }
  ...
  async editRoleApplication(
    ...
    var application = await RoleApplication.findOne(id, {
      relations: ["role"],
    });
    ...
    if (!application) {
      return {
        errors: [
          { field: "Role Application ID", message: "No application found" },
        ],
      };
    } else {
      return { application };
